...
import { addDays, sameDay, diffDays } from "./core";
...
function buildBrand(ctx, bid, d) {
  ...
  if (isEventDay(ctx, bid, d)) {
    tasks.push({ ch: "story", type: "סיקור לייב", flag: "urgent", title: "🎉 יום האירוע — סיקור לייב לאורך הערב (סטוריז)" });
    return tasks;
  }
  ...
  // ── weekly rhythm ──
  if (wd === 0) {
    tasks.push({ ch: "post", type: "פוסט", title: "פוסט שבועי — קרוסלת תמונות / ריל אווירה" });
