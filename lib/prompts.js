...
export function cleanMessage(text) {
  ...
  // Normalize asterisk escaping so every * is a uniform WhatsApp-bold \*, no matter
  // how (in)consistently the model escaped it. Two ordered steps:
  //   1. drop ALL backslashes → also removes the model's bogus \! \. \, \-
  //   2. add one backslash before every * → \*, \*טקסט*, *טקסט* all become \*…\*
  // Order matters: stripping first prevents double escapes (\\*).
  t = t.replace(/\\/g, "");
  t = t.replace(/\*/g, "\\*");
