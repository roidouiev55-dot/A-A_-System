...
export const validate = {
  ...
  // Content Trainer
  trainerBrand: (b) => {
    const brand = T.brand()(b && b.brand);
    if (!brand) throw new ValidationError("מותג לא תקין");
    return brand;
  },
  trainerFeedback: (b) => {
    const brand = T.brand()(b && b.brand);
    if (!brand) throw new ValidationError("מותג לא תקין");
    const decision = T.oneOf(["approved", "rejected"])(b && b.decision);
    if (!decision) throw new ValidationError("החלטה לא תקינה");
    const note = T.text(2000)(b && b.note);
    return {
      brand,
      content_type: T.text(100)(b && b.content_type) || "",
      suggestion: T.text(5000)(b && b.suggestion) || "",
      decision,
      note: note ? note : null,
    };
  },

  // single global row, fixed id so upsert always targets the same note
  personalNote: (b) => ({ id: "main", content: T.text(20000)((b && b.content) ?? "") || "" }),
