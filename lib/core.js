...
// ── Israel-anchored formatters (timezone-safe) ──
// The formatters above read local date parts (getDate/getMonth/getDay), so they
// only give the right answer where the runtime clock is Israel time — i.e. the
// browser. On a UTC server (Vercel) an event stored as Israel-midnight is really
// the previous day 21:00Z, so getDate() returns the day BEFORE. These variants
// pin the calendar to Asia/Jerusalem via Intl, so they read correctly whether the
// code runs in the browser or on a UTC server. Use them for anything formatted
// server-side (Gemini prompts, API routes).
const IL_TZ = "Asia/Jerusalem";
const IL_DOW = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
function ilParts(d) {
  const parts = {};
  for (const p of new Intl.DateTimeFormat("en-US", {
    timeZone: IL_TZ, weekday: "short", year: "numeric", month: "2-digit", day: "2-digit",
  }).formatToParts(new Date(d))) parts[p.type] = p.value;
  return { day: parts.day, month: parts.month, year: parts.year, dow: IL_DOW.indexOf(parts.weekday) };
}
export function fmtDateFullIL(d) { const p = ilParts(d); return `${p.day}/${p.month}/${p.year}`; }
export function dowHebIL(d) { return HEB_DAYS[ilParts(d).dow]; }
